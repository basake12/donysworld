...
export function ModelProfileClient({ profile, user, pendingEmailChange }: ProfileClientProps) {
  ...
  async function onEmailSubmit(data: EmailFormData) {
    setSavingEmail(true);
    try {
      const res = await fetch("/api/model/email/request-change", {
      ...
      toast({ title: "Couldn't start email change", description: msg, variant: "destructive" });
    } finally {
    ...
    <div className="space-y-8 max-w-2xl">
      ...
      <form onSubmit={emailForm.handleSubmit(onEmailSubmit)}>
        <div className="rounded-xl border border-border bg-card p-5 space-y-5">
          ...
          {pendingEmailChange && (
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 space-y-1">
              <p className="text-xs font-bold text-amber-400">
                Verification pending for {pendingEmailChange.newEmail}
              </p>
              <p className="text-[11px] text-muted-foreground">
                Check that inbox for the confirmation link. The request expires{" "}
                {new Date(pendingEmailChange.expiresAt).toLocaleString("en-NG", { dateStyle: "short", timeStyle: "short" })}.
                Submitting a new change below will cancel this request.
              </p>
            </div>
          )}
          ...
          <p className="text-[11px] text-muted-foreground">
            We&rsquo;ll send a confirmation link to the new address. Your email won&rsquo;t change until you click it.
          </p>
        </div>

        <Button type="submit" disabled={savingEmail} className="w-full h-11 bg-gold-gradient text-primary-foreground font-semibold hover:opacity-90 gold-glow mt-4">
          {savingEmail ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Mail className="mr-2 h-4 w-4" />Send Confirmation Link</>}
        </Button>
      </form>
        ...
        {gallery.length < 6 && (
          <label htmlFor="galleryUpload" className={cn(
            ...
            <p className="text-xs text-muted-foreground text-center">
              {uploadingGallery ? "Processing..." : "Add photo · Face auto-blurs"}
              <br /><span className="text-[10px]">JPG, PNG, WebP · Max 10MB</span>
            </p>
